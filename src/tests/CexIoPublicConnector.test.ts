...
import { CexIoPublicConnector } from '../connectors';
import { getCexIoSymbol, Logger } from '../utils';

const logger = Logger.getInstance('test-public-connector');

describe('CexIoPublicConnector', () => {
  ...
  let server: WSS;
  let connector: CexIoPublicConnector;
  const symbol = getCexIoSymbol(mockGroup, mockConfig);

  beforeEach(async () => {
    server = new WSS({ port: 12345 });
    connector = new CexIoPublicConnector(mockGroup, mockConfig);
  });

  afterEach(async () => {
    if (
      connector.publicWsFeed &&
      connector.publicWsFeed.readyState === WebSocket.OPEN
    ) {
